...
| Storage synchronization capability (`SyncCapability::{kUnsupported,kVolatileNoop,kPowerLossDurable}`) | Normative | Planned | [ADR-0034](adr/0034-storage-durability-barrier.md) | — | Values 0/1/2 are stable and append-only; `kVolatileNoop` is ordering-only, and only `kPowerLossDurable` qualifies a durable synchronized Fence |
